...
contract EnglishAuction {
  ...
  IERC20 public immutable daToken;
    
  constructor(address _daTokenAddress) {
      if (_daTokenAddress == address(0)) revert EnglishAuction__InvalidAddress();
      daToken = IERC20(_daTokenAddress);
  }
  ...
  function bid(
    ...
    // 转移 DAToken 到合约
    bool success = daToken.transferFrom(msg.sender, address(this), _bidAmount);
    if (!success) revert EnglishAuction__TransactionFailed();

    // 如果存在之前的出价，退还给前一个出价者
    if (highestBidder != address(0)) {
        success = daToken.transfer(highestBidder, highestBid);
        if (!success) revert EnglishAuction__TransactionFailed();
    }
  ...
  function endAuction(
        ...
        // 转移 DAToken 给卖家
        bool success = daToken.transfer(seller, highestBid);
        if (!success) revert EnglishAuction__TransactionFailed();
